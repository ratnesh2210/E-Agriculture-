...
<div class="btn-group pull-right">
  ...
  <?php 
    $counter=0;
    $sql=mysqli_query($connect,"SELECT count(*) from cart where customer_id='$user' AND status='Not Purchased'");
    $row=mysqli_fetch_row($sql);
    $counter=$row[0];
    if($counter>0)
    {
      ?>
      <a class="pull-right" href="cart_details.php"><button class="btn btn-success"><?php echo $counter;?> Product in Cart</button></a>
    <?php
    }
    else
    {
      ?>
      <a class="pull-right" href="cart_details.php"><button class="btn btn-success">Your Cart</button></a>
    <?php
    }
  ?>
</div>
 ...
 <div class="container-fluid text-center bg-grey">
    <div class="row">
<?php
  $sql=mysqli_query($connect,"SELECT * from upload_db where product_name LIKE '$name%' OR product_name LIKE '%$name%' OR product_name LIKE '%$name'") or die("cannot do sql".mysqli_error($connect));
  $counter=1;
  ...
  <?php
    ...
    <div class = "col-sm-6 col-md-3">
      <div class = "thumbnail">
        ...
        <strong>Selling Price:</strong><?php echo $row['selling_price']?><br>
        <strong>Validity:</strong><?php echo $row['validity']?><br>
        <a href="cart.php?product_id=<?php echo $row['product_id'];?>"><button type="submit" class="btn btn-primary">Add to cart</button>
        </div></a>
      </div>
    </div>
    <?php
